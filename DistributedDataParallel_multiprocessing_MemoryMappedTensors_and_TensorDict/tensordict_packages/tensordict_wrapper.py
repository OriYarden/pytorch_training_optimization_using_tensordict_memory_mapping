...
def dataset_to_tensordict(ds, DEVICE='cpu', list_of_dtypes=None, force_list_of_tensordict_keys=[]):
    ...
    if one wants to set the dtypes, and force_list_of_tensordict_keys if one wants to set the keys manually.
    ...
    _____
        ...
        - force_list_of_tensordict_keys: optional [List] of names [str] of the variables returned from
            the torch.utils.data.Dataset.__getitem__ return statement to use to as memory mapped keys.
            Defaults to using get_var_names_returned_from_getitem_method function from utils_and_toolbox
            to obtain the keys for memory mapping. If Dataset is wrapped with Sampler (or similarly), then
            force_list_of_tensordict_keys should be used to manually override key-mapping.
    ...
    list_of_tensordict_keys = get_var_names_returned_from_getitem_method(
        ds=ds
    ) if not force_list_of_tensordict_keys else force_list_of_tensordict_keys
